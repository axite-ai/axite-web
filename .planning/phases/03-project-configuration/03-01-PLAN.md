...
Note: lib/config/ is pure JS, doesn't need tsconfig.
  ...
  <verify>
For each lib package with TS files, verify extends path resolves:
```bash
for pkg in ui common icons shared-data ui-patterns api-types; do
  if [ -f "lib/$pkg/tsconfig.json" ]; then
    echo "Checking lib/$pkg..."
    cat "lib/$pkg/tsconfig.json" | grep -o '"extends"[^,]*'
  fi
done
```
  </verify>
