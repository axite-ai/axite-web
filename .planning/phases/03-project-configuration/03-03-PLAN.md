...
- outputFileTracingExcludes - good optimization
  ...
  <verify>
Node can parse the config:
```bash
cd apps/www && node -e "import('./next.config.mjs').then(c => console.log('Next config loads OK'))"
```
  </verify>
  <done>next.config.mjs is standalone, doesn't reference Sentry or missing env vars</done>
</task>
...
5. getAssetPrefix() doesn't reference undefined env vars
</verification>
