...
1. Parse pnpm-workspace.yaml to extract catalog: version mappings (using simple string parsing - NO external yaml library since pnpm can't run yet)
...
// Simple YAML parsing for catalog section (no external dependencies needed)
function parseSimpleYaml(content) {
  ...
  for (const line of lines) {
    ...
    // Named catalogs section (catalogs:)
    if (line.trim() === 'catalogs:') {
      currentSection = 'catalogs';
      continue;
    }
    // Named catalog entry (e.g., "  react19:")
    if (currentSection === 'catalogs' && line.match(/^  \w+:$/)) {
      currentCatalogName = line.trim().replace(':', '');
      catalogs[currentCatalogName] = {};
      continue;
    }
    // Package entry in catalog or named catalog
    if (currentSection === 'catalog' && line.startsWith('  ') && line.includes(':')) {
      const match = line.match(/^\s+['"]?([^'":\s]+)['"]?\s*:\s*['"]?([^'"#\s]+)['"]?/);
      if (match) {
        catalog[match[1]] = match[2];
      }
    } else if (currentSection === 'catalogs' && currentCatalogName && line.startsWith('    ') && line.includes(':')) {
      const match = line.match(/^\s+['"]?([^'":\s]+)['"]?\s*:\s*['"]?([^'"#\s]+)['"]?/);
      if (match) {
        catalogs[currentCatalogName][match[1]] = match[2];
      }
    } else if (!line.startsWith(' ') && line.trim() && !line.startsWith('#')) {
      // New top-level section, reset
      currentSection = null;
...
const { catalog, catalogs } = parseSimpleYaml(workspaceContent);

console.log('Loaded catalog with', Object.keys(catalog).length, 'entries');
if (Object.keys(catalogs).length > 0) {
  console.log('Loaded named catalogs:', Object.keys(catalogs).join(', '));
}
...
Run the script (uses only Node.js built-ins, no external dependencies needed):
```bash
