...
/**
 * Color token stub - replaces missing build artifact from lib/ui/build/css/tw-extend/color
 *
 * The original Supabase design system generated this from Figma tokens via Style Dictionary.
 * This stub provides the CSS variable references that the typography config expects.
 * The actual CSS variable definitions must be provided by the app's global CSS.
 *
 * Format: { 'color-name': { cssVariable: 'var(--color-name)' } }
 * The typography plugin transforms these into: hsl(var(--color-name) / <alpha-value>)
 */
const color = {
  ...
  'background-surface-75': { cssVariable: 'var(--background-surface-75)' },
  ...
  'border-stronger': { cssVariable: 'var(--border-stronger)' },
